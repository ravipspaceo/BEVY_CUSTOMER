...
-(int)heightForText:(NSDictionary*)buttonInfo {
    ...
    {
        CGSize  textSize = CGSizeMake(180, 10000.0);
        CGRect textRect = [buttonText boundingRectWithSize:textSize
                                                options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading)
                                             attributes:@{NSFontAttributeName:[GlobalManager fontMuseoSans300:16]}
                                                context:nil];
        
        newSize = textRect.size;
    }
    else{
        CGSize  textSize = CGSizeMake(200, 10000.0);
        newSize = [buttonText sizeWithFont:[GlobalManager fontMuseoSans300:16]
            ...
            for (int n = 0; n < _buttonsStrings.count; n++)
            {
                ...
                for (int i = 0; i < linesArr.count; i++) {
                    ...
                    if(buttonString.range.location >= range.location && buttonString.range.location < (range.location + range.length)) {
                        ...
                        CGRect buttonFrame = CGRectMake(startFloat + origins[i].x, (CGRectGetHeight(self.bounds) - fontSize - origins[i].y + 2.0), [self getWidthForButtonAttributes:buttonInfo], [self heightForText:buttonInfo]);
