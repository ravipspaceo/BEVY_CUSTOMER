...
//  Miso
//
//  Created by CompanyName on 11/15/11.
...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    if (numLines > self.numberOfLines && self.numberOfLines != 0) {
        numLines = ( int)self.numberOfLines;
    }
    ...
    for (int i = 0; i < numLines; i++) {
        ...
        if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")) {
            if (self.textAlignment == NSTextAlignmentCenter) {
                drawX = floorf((self.frame.size.width - [line  sizeWithAttributes:@{NSFontAttributeName:self.font}].width) / 2);
            } else if (self.textAlignment == NSTextAlignmentRight) {
                drawX = (self.frame.size.width - [line  sizeWithAttributes:@{NSFontAttributeName:self.font}].width);
            }
        } else {
            if (self.textAlignment == NSTextAlignmentCenter) {
...
- (NSArray *)stringsFromText:(NSString *)string {
    ...
    if (self.lineBreakMode == (SYSTEM_VERSION_LESS_THAN(@"6.0") ? NSLineBreakByWordWrapping : NSLineBreakByWordWrapping)) {
