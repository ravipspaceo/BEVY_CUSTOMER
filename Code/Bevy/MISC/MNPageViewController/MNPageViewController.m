...
- (void)viewDidLoad
{
    ...
    CGRect bounds = self.view.bounds;//CGRectMake(self.view.bounds.origin.x, self.view.bounds.origin.y, self.view.bounds.size.width, self.view.bounds.size.height + 10);
...
{
    if ([self.delegate respondsToSelector:@selector(mn_pageViewController:willBeginPageToViewController:)])
    {
        [self.delegate mn_pageViewController:self willBeginPageToViewController:self.viewController];
    }
}
